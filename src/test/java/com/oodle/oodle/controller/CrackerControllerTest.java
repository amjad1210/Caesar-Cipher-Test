...
class CrackerControllerTest {
    ...
    void crackShouldReturnValidResponse() {
        ...
        var request1 = restTemplate.postForObject("http://localhost:" + port + "/crack", body1, String.class);
        assertEquals("{\"plaintext\":\"THISISASECRETMESSAGE\"}", request1);

        var body2 = new EncryptedMessage("NKRRUCUXRJ", new String[]{"HELLO", "WORLD"});
        var request2 = restTemplate.postForObject("http://localhost:" + port + "/crack", body2, String.class);
        assertEquals("{\"plaintext\":\"HELLOWORLD\"}", request2);

        var body3 = new EncryptedMessage("CNQILEM", new String[]{"IT", "WORK"});
        var request3 = restTemplate.postForObject("http://localhost:" + port + "/crack", body3, String.class);
